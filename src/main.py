...
def _post_init(self):
    ...
    """
    logger.debug("Calling the post_init method in handler_rmrxapp-python")
...
def _default_handler(self, summary, sbuf):
    ...
    logger.debug("Handler_rmrxapp.A1PolicyHandler.default_handler called for msg type = " + str(summary[rmr.RMR_MS_MSG_TYPE]))
    logger.debug("Handler_rmrxapp.A1PolicyHandler.default_handler called and says:: Received summary is {}".format(summary))
    self.rmr_free(sbuf)
...
def start(thread = False):
    ...
    logger.debug("Handler_rmrxapp is starting.......")
    global handler_rmrxapp 
    fake_sdl = getenv("USE_FAKE_SDL", None)
    handler_rmrxapp = RMRXapp(default_handler = _default_handler,
                       rmr_port=4560,
                       post_init=_post_init,
                       use_fake_sdl=bool(fake_sdl))
    logger.debug("Handler_rmrxapp created .......")
    A1PolicyHandler(handler_rmrxapp, Constants.A1_POLICY_REQ) # NEED TO CHECK THIS OUT: if A1_POLICY_REQ equals A1_POLICY_QUERY, then the handler for that query will be called else the default handler will be called
    #a1_mgr = A1PolicyManager(handler_rmrxapp)
    #a1_mgr.startup()
